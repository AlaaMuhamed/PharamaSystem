...
extension HomeSceneView {
    ...
    @ViewBuilder private func makePharmacyListView() -> some View {
        ScrollView {
            LazyVStack(alignment: .leading) {
                ForEach(viewModel.filteredPharmacies, id: \.id) { pharmacy in
                    Button(action: {
                        coordinator?.routeToRepoDetails(repoDetails: pharmacy)
                    }) {
                        PharmacyBannerView(name: pharmacy.doingBusinessAs ?? "")
                    }
