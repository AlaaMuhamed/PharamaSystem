...
final class HomeCoordinator: RouterCoordinator {
    ...
    func routeToRepoDetails(repoDetails: PharmacyListResponseDataModel) {
        let viewModel = PharmacyDetailsViewModel()
        viewModel.pharmacy = repoDetails
        let child = PharmacyDetailsCoordinator(router: router, viewModel: viewModel)
